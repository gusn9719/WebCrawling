...
> **ReviewFit** · 기말 프로젝트 — 화장품 리뷰 크롤링 → 전처리 → 감성 분석 모델 학습 → Streamlit 배포
...
이 프로젝트에서 생성한 `product_recommendation_scores.parquet`와 `service_reviews.parquet`는 후속 [BeautyLens](https://github.com/gusn9719/beautylens-mvc)에서 Oracle DB에 임포트되어 Spring MVC 서비스의 상품·리뷰 데이터로 사용됐다. 감성 분석 모델 자체를 Spring에 배포한 구조가 아니라, 이 파이프라인이 만든 추천·리뷰 artifact를 서비스 데이터로 연결한 관계다.
